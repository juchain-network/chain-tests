...
scenario_init "add-validator-live"
...
echo "[scenario/add-validator-live] prepare multi-validator separated-signer network (epoch=$SCENARIO_EPOCH)"
...
(
  cd "$ROOT_DIR"
  go test ./tests/epoch -run TestZ_AddValidatorWithSeparateSignerBecomesActiveAndSealsBlocks -count=1 -timeout 20m
)

scenario_mark_stage "completed"
bash "$ROOT_DIR/scripts/network/native.sh" down "$SESSION_FILE"

echo "[scenario/add-validator-live] 🟢 PASS"
