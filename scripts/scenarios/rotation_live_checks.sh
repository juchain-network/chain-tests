...
echo "[scenario/rotation-live] prepare multi-validator separated-signer network (epoch=$SCENARIO_EPOCH)"
...
(
  cd "$ROOT_DIR"
  go test ./tests/epoch -run TestZ_SignerRotationNewSignerContinuesSealingAfterCheckpoint -count=1 -timeout 20m
)

scenario_mark_stage "completed"
scenario_network down

echo "[scenario/rotation-live] 🟢 PASS"
