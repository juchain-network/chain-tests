...
func runExecutePendingAutoByConsensus(t *testing.T, label string) {
	t.Helper()
	if ctx == nil {
		t.Fatalf("Context not initialized")
	}
	...
	if checkpoint <= currentHeight+stopLead+1 {
		checkpoint += epoch
	}
	stopHeight := checkpoint - stopLead
	if stopHeight <= currentHeight {
		t.Fatalf("invalid stop window: current=%d stop=%d checkpoint=%d stop_lead=%d", currentHeight, stopHeight, checkpoint, stopLead)
	}
	...
	if _, has, err := pendingHeadAt(0); err != nil {
		t.Fatalf("read pending queue before %s failed: %v", label, err)
	} else if has {
		t.Fatalf("pending queue should be empty before %s assertion", label)
	}

	t.Logf(
		"%s target validator=%s signer=%s stop_height=%d checkpoint=%d stop_lead=%d signers=%v",
		label,
		targetVal.Hex(),
		targetSigner.Hex(),
		stopHeight,
	...
	t.Cleanup(func() {
		if err := testkit.RestartValidatorNodeWithSigner(ctx, targetVal, targetSignerKey, 90*time.Second); err != nil {
			t.Errorf("restart validator %s after %s failed: %v", targetVal.Hex(), label, err)
		}
	})
	heightBeforeStop, err := testkit.WaitUntilClientHeight(
		ctx.Clients[0],
		label+" wait until stop window",
		stopHeight,
	...
	if postStopHeader.Number.Uint64() >= checkpoint {
		t.Fatalf(
			"stop completed too late to force checkpoint miss: checkpoint=%d head_after_stop=%d signer=%s",
			checkpoint,
			postStopHeader.Number.Uint64(),
			targetSigner.Hex(),
		)
	}
	...
	if _, err := testkit.WaitUntilHeightOrStall(
		ctx,
		label+" post-stop chain recovery",
		beforeProgress+1,
		15*time.Second,
		30*time.Second,
	); err != nil {
	...
	if _, err := testkit.WaitUntilHeightOrStall(
		ctx,
		label+" executePending auto-consume",
		checkpoint+1,
		20*time.Second,
	...
	if checkpointHeader.Coinbase == targetSigner {
		t.Fatalf(
			"target signer still sealed checkpoint block; stop happened too late: checkpoint=%d signer=%s hash=%s",
			checkpoint,
			targetSigner.Hex(),
			checkpointHeader.Hash().Hex(),
		)
	}
...
func TestZ_ExecutePendingAutoByConsensus(t *testing.T) {
	runExecutePendingAutoByConsensus(t, "P-24 isolated")
}
